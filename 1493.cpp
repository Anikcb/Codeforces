...
map<ll,set<pll> >store;
mll res;

void seive()
{
    ll i,j;
    for(i=1;i<=mxx;i++)arr[i]=i;
    for(i=4;i<mxx;i+=2)arr[i]=2;
...
void check(ll n,ll p)
{
    while(n!=1)
    {
        ...
        while(n%x==0)cnt++,n/=x;
        freq[p][x]+=cnt;
        store[x].insert(MP(cnt,MP(x,p)));
    }
}

void check2(ll n,ll p,ll sz)
{
    while(n!=1)
    {
        ll prime=arr[n],cnt=0;
        while(n%prime==0)n/=prime,cnt++;
        ll val=freq[p][prime];
        store[prime].erase(MP(val,MP(prime,p)));
        store[prime].insert(MP(val+cnt,MP(prime,p)));
        if(SZ(store[prime])==sz)
        {
            pll beg=*store[prime].begin();
            ll dif=beg.ff-res[prime];
            foR1(dif)ans=(ans*prime)%MOD;
            res[prime]+=dif;
        }
...
int main()
{
    ...
    for(ll tt=1; tt<=tst; tt++)
    {
        ...
        ll val=ans;
        while(val>1)res[arr[val]]++,val/=arr[val];
        while(q--)
        {
            cin>>a>>b;
            check2(b,a,n);
